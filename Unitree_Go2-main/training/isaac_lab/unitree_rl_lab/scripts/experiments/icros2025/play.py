...
def main():
    ...
    def override_cmd_and_obs(obs_tensor):
        """Override command manager tensor AND the obs tensor so policy sees fixed cmd.
        Policy obs layout: base_ang_vel(0:3), projected_gravity(3:6), velocity_commands(6:9), ...
        """
        if use_fixed_cmd:
            ...
            cmd[:, 1] = fixed_vy
            cmd[:, 2] = fixed_wz
            # Clone to allow in-place update (obs may be an inference tensor)
            obs_tensor = obs_tensor.clone()
            obs_tensor[:, 6] = fixed_vx
            obs_tensor[:, 7] = fixed_vy
            obs_tensor[:, 8] = fixed_wz
        return obs_tensor
