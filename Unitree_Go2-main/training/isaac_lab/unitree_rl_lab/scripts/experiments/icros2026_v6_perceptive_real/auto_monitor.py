#!/usr/bin/env python3
"""V6 Auto Monitor — Checkpoint-based Phase Transitions (V4 log-count 버그 수정).
...
## V6 수정 사항
  ...
  - TOTAL_ITER=50000, V6 scratch 학습을 기본 경로로 사용
...
NUM_ENVS     = 16384
LOG_FILE     = "/tmp/train_icros2026_v6_perceptive_real.log"
LOG_DIR      = "/workspace/logs/rsl_rl/unitree_go2_icros2026_v6_perceptive_real"
OVERRIDE_PATH = "/tmp/training_override.json"
STATE_FILE   = "/tmp/monitor_state_icros2026_v6_perceptive_real.json"
DONE_FILE    = "/tmp/monitor_done_icros2026_v6_perceptive_real"

TRAIN_SCRIPT = "/workspace/unitree_rl_lab/scripts/experiments/icros2026_v6_perceptive_real/train.py"
...
PHASES = [
    ...
    },
    {
        ...
        "move_up_m": 2.5,
        "move_down_m": 0.6,
        "desc": "고난도 안정화: V5보다 속도 압박 완화",
    },
    {
        "name": "Phase4-HardConsolidation",
        "start_ckpt": 28000,
        "max_init_level": 8,
        "min_terrain_level": 5,
        "lin_vel_x_max": 1.4,
        ...
        "move_up_m": 2.4,
        "move_down_m": 0.6,
        "desc": "V5 plateau 이후 level 5+ 강제 노출, 속도보다 돌파 우선",
    },
    {
        "name": "Phase5-Level7Exposure",
        "start_ckpt": 38000,
        "max_init_level": 9,
        "min_terrain_level": 6,
        "lin_vel_x_max": 1.5,
        ...
        "move_up_m": 2.3,
        "move_down_m": 0.6,
        "desc": "level 6+ 노출 확대, level 8 진입 준비",
    },
    {
        "name": "Phase6-Level8Push",
        "start_ckpt": 45000,
        "max_init_level": 9,
        "min_terrain_level": 7,
        "lin_vel_x_max": 1.6,
        ...
        "move_up_m": 2.2,
        "move_down_m": 0.55,
        "desc": "level 7+ 상시 노출로 평균 8+ 돌파 시도",
    },
]
...
def get_training_pids() -> list[int]:
    """컨테이너에서 V6 train.py 관련 프로세스 PID 목록을 반환."""
    out = docker_exec(
        "ps -eo pid=,args= "
...
def get_training_python_pids() -> list[int]:
    """실제 V6 학습 Python 프로세스 PID 목록을 반환."""
    out = docker_exec(
        "ps -eo pid=,args= "
        f"| grep -F '{TRAIN_SCRIPT}' "
        f"| grep -F '{TASK}' "
...
def is_training_running() -> bool:
    """컨테이너에서 V6 train.py 프로세스가 실행 중인지 확인."""
...
def stop_training(force: bool = False):
    ...
    if not pids:
        log("  종료할 V6 학습 프로세스 없음")
        return

    pid_text = " ".join(str(pid) for pid in pids)
    log(f"  V6 학습 종료 요청: pids={pid_text}, force={force}")
    ...
    if force or remaining:
        if remaining:
            ...
            log(f"  남은 V6 학습 강제 종료: pids={rem_text}")
...
def write_override(
    ...
    vel_min: float,
    dr_phase: int,
    min_terrain_level: int = 0,
    move_up_m: float = 2.4,
    move_down_m: float = 0.6,
):
...
def _phase_override_args(phase: dict) -> tuple[int, float, float, int, int, float, float]:
    return (
        phase["max_init_level"],
        phase["lin_vel_x_max"],
        phase["lin_vel_x_min"],
        phase["dr_phase"],
        phase.get("min_terrain_level", 0),
        phase.get("move_up_m", 2.4),
        phase.get("move_down_m", 0.6),
    )


def _checkpoint_iter_from_name(ckpt_file: str) -> int:
...
def start_training(ckpt_path: str = "") -> bool:
    ...
    if ckpt_path:
        ...
        ckpt_iter = _checkpoint_iter_from_name(ckpt_file)
...
def main():
    ...
    write_override(
        *_phase_override_args(cur_phase),
    )
    ...
    while True:
        ...
        if ckpt_iter >= TOTAL_ITER - 1:
            log(f"🎉 학습 완료! (ckpt_iter={ckpt_iter}) Monitor 종료.")
        ...
        if len(python_pids) > 1:
            ...
            log(f"🔴 V6 학습 Python 프로세스 중복 감지! python_pids={python_pids}, all_pids={all_pids} → 최신 ckpt에서 단일 재시작")
            cur_phase_idx, cur_phase = get_current_phase(ckpt_iter)
            write_override(*_phase_override_args(cur_phase))
        ...
        if new_phase_idx != state["current_phase_idx"]:
            ...
            write_override(*_phase_override_args(new_phase))
        ...
        if is_training_running():
            if ckpt_iter > state["last_ckpt_iter"]:
                ...
                if stall_secs >= effective_timeout:
                    ...
                    write_override(*_phase_override_args(cur_ph))
                    start_training(ckpt_path)
                    stall_count = 0
        else:
            # 프로세스 없음 → 크래시
